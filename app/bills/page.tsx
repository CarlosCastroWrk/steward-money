...
function detectRecurring(
  ...
  for (const [key, txs] of grouped) {
    ...
    const alreadyBill = existingNames.some((name) => name.toLowerCase().includes(key) || key.includes(name.toLowerCase()));
    if (alreadyBill) continue;
    const avgAmount = txs.reduce((s, tx) => s + Math.abs(tx.amount), 0) / txs.length;
    suggestions.push({ merchant: txs[0].merchant ?? key, amount: Math.round(avgAmount * 100) / 100, frequency, occurrences: txs.length });
  }
...
export default async function BillsPage() {
  ...
  const [billsRes, accountsRes, txRes] = await Promise.all([
    supabase.from("bills").select("id, user_id, name, amount, due_day, frequency, is_autopay, next_due_date, account_id, notes, created_at, paid_at, auto_detected_paid").eq("user_id", user.id).order("next_due_date", { ascending: true, nullsFirst: false }),
    supabase.from("accounts").select("id, name, type").eq("user_id", user.id).eq("is_active", true),
    supabase.from("transactions").select("merchant, amount, date").eq("user_id", user.id).eq("is_manual", false).gte("date", startDate),
  ]);
  ...
  for (const b of bills) {
    ...
    if (b.paid_at) {
      ...
      if (paidDate >= new Date(monthStart) && paidDate <= new Date(monthEnd + "T23:59:59")) {
    ...
    if (!b.paid_at && b.next_due_date && b.next_due_date >= todayStr) {
      ...
      if (!nextBill || diff < nextBill.daysUntil) {
        nextBill = { name: b.name, daysUntil: diff };
      }
    }
  }
